<div class="page">

# MACSEC en punto a punto

\

# MACSEC PUNTO A PUNTO

\#Creamos claves

<div class="codebox">

    dd if=/dev/urandom count=16 bs=1 2> /dev/null | hexdump -e '1/2 "%04x"'
    dd if=/dev/urandom count=32 bs=1 2> /dev/null | hexdump -e '1/2 "%04x"'

</div>

En UBUNTU SERVER hay que instalar NMCLI:

<div class="codebox">

    sudo apt install network-manager

</div>

Modificar el NETPLAN para incluir las dos primeras líneas, version y
renderer:

<div class="codebox">

    network:
      version: 2
      renderer: NetworkManager
      ethernets:

</div>

Comprobamos la configuración:

<div class="codebox">

    sudo nano /etc/NetworkManager/NetworkManager.conf

</div>

Debe aparecer al menos:

<div class="codebox">

    main]
    plugins=keyfile

</div>

Y reiniciamos el NetworkManager:

<div class="codebox">

    sudo systemctl restart NetworkManager

</div>

Hay que incluir macsec como dispositivo no gestionado

<div class="codebox">

    sudo nano /usr/lib/NetworkManager/conf.d/10-globally-managed-devices.conf

</div>

Añadimos.

<div class="codebox">

     [keyfile]
    unmanaged-devices=*,except:type:wifi,except:type:ethernet

</div>

Y recargamos:

<div class="codebox">

    sudo systemctl reload NetworkManager

</div>

C

## on nmcli

nmcli connection add type macsec con-name macsec0 ifname macsec0
connection.autoconnect yes macsec.parent enp0s8 macsec.mode psk
macsec.mka-cak 50b71a8ef0bd5751ea76de6d6c98c03a macsec.mka-ckn
f2b4297d39da7330910a74abc0449feb45b5c0b9fc23df1430e1898fcf1c4550

nmcli connection modify macsec0 ipv4.method manual ipv4.addresses
'192.0.2.1/24' ipv4.gateway '192.0.2.254' ipv4.dns '192.0.2.253'

1.  Verify that the traffic is encrypted:

`# tcpdump -nn -i enp1s0`

1.  Optional: Display the unencrypted traffic:

`# tcpdump -nn -i macsec0`

1.  Display MACsec statistics:

`# ip macsec show`

1.  Display individual counters for each type of protection:
    integrity-only (encrypt off) and encryption (encrypt on)

`# ip -s macsec show`

</div>
